...
object GIB{
   ...
   * @param pin2track    pin-to-track connection flexibility
   */
  def getOPin2TrackConnect(numTrack: Int, numPinList: List[Int], pin2track: List[Int], trackDirections : ListBuffer[Int]): ListBuffer[Seq[Int]] = {
    // numPinList
...
//    }
    ...
    for(i <- 0 until numNWpin){
      for (j <- 0 until fcPerSide(0)) {
        val fcStep = w.toFloat / fcPerSide(0)
        val pinStep = fcStep / numNWpin
        val index = (i * pinStep + j * fcStep).toInt
        connect += Seq(NORTHWEST, i, NORTH, rule((NORTHWEST, NORTH))(Seq(index, w)))
      }
      for (j <- 0 until fcPerSide(1)) {
        val fcStep = w.toFloat / fcPerSide(1)
        val pinStep = fcStep / numNWpin
        val index = (i * pinStep + j * fcStep).toInt
        connect += Seq(NORTHWEST, i, EAST, rule((NORTHWEST, EAST))(Seq(index, w)))
      }
      for (j <- 0 until fcPerSide(2)) {
        val fcStep = w.toFloat / fcPerSide(2)
        val pinStep = fcStep / numNWpin
        val index = (i * pinStep + j * fcStep).toInt
        connect += Seq(NORTHWEST, i, SOUTH, rule((NORTHWEST, SOUTH))(Seq(index, w)))
      }
      for (j <- 0 until fcPerSide(3)) {
        val fcStep = w.toFloat / fcPerSide(3)
        val pinStep = fcStep / numNWpin
        val index = (i * pinStep + j * fcStep).toInt
...
//      }
   ...
   * @param track2pin   track-to-pin connection flexibility
   */
  def getTrack2IPinConnect(numTrack: Int, numPinList: List[Int], track2pin:List[Int], trackDirections : ListBuffer[Int]): ListBuffer[Seq[Int]] = {
    // numPinList, the input pins of PE/LSU (output pins of GIB)
    val numNWpin = numPinList(0)  // NORTHWEST side
    val numNEpin = numPinList(1)  // NORTHEAST side
    val numSEpin = numPinList(2)  // SOUTHEAST side
    val numSWpin = numPinList(3)  // SOUTHWEST side
    //println("trackDirections: " + trackDirections)
    val w = numTrack
    val fcPerSide = track2pin
//    val fcStep = w.toFloat/fc
    var connect = ListBuffer[Seq[Int]]() // (srcSide, srcTrack, dstSide, dstTrack)
//    var pinStep = {
//      if(numNWpin > 0) fcStep/numNWpin else 0.0
...
//      }
    ...
    /*for(i <- 0 until connect.size){
      println("connect: " + connect(i))
    }
    connect.distinct.filter(ins => !((ins(0) <= 3 && trackDirections(ins(0)) == 0) || (ins(2) <= 3 && trackDirections(ins(2)) == 0)))
    for (i <- 0 until connect.size) {
      println("fileter connect: " + connect(i))
    }*/
    connect.distinct.filter(ins => !((ins(0) <= 3 && trackDirections(ins(0)) == 0) || (ins(2) <= 3 && trackDirections(ins(2)) == 0)))
  }
   ...
   * @param pin2pin  pin-to-pin connection flexibility
   */
  def getOPin2IPinConnect(numIOPinList: List[Int], pin2pin: List[Int]): ListBuffer[Seq[Int]] = {
    // numIOPinList
...
//    val fcSWstep = nSWi/fcPerSide.toFloat
    for(i <- 0 until nNWo){
        for(j <- 0 until fcPerSide(0)){
          if(nNEi > 0) {
            val fcNEstep = nNEi / fcPerSide(0).toFloat
      ...
      for (j <- 0 until fcPerSide(1)) {
        if (nSEi > 0) {
          val fcSEstep = nSEi / fcPerSide(1).toFloat
      ...
      for (j <- 0 until fcPerSide(2)) {
        if (nSWi > 0) {
          val fcSWstep = nSWi / fcPerSide(2).toFloat
...
//          val iNE = (i * fcNEstep / nNWo + j * fcNEstep).toInt
//          connect += Seq(NORTHWEST, i, NORTHEAST, rule((NORTHWEST, NORTHEAST))(Seq(iNE, nNEi)))
//          //println("NORTHWEST i:" + i + " NORTHWEST:" + rule((NORTHWEST, NORTHEAST))(Seq(iNE, nNEi)))//@HETA: nNEi can be treat as w
//        }
...
//      }
    ...
    for(i <- 0 until nNEo){
      for (j <- 0 until fcPerSide(0)) {
        if (nSEi > 0) {
          val fcSEstep = nSEi / fcPerSide(0).toFloat
      ...
      for (j <- 0 until fcPerSide(1)) {
        if (nSWi > 0) {
          val fcSWstep = nSWi / fcPerSide(1).toFloat
      ...
      for (j <- 0 until fcPerSide(2)) {
        if (nNWi > 0) {
          val fcNWstep = nNWi / fcPerSide(2).toFloat
...
//      }
    ...
    for(i <- 0 until nSEo){
      for (j <- 0 until fcPerSide(0)) {
        if (nSWi > 0) {
          val fcSWstep = nSWi / fcPerSide(0).toFloat
      ...
      for (j <- 0 until fcPerSide(1)) {
        if (nNWi > 0) {
          val fcNWstep = nNWi / fcPerSide(1).toFloat
      ...
      for (j <- 0 until fcPerSide(2)) {
        if (nNEi > 0) {
          val fcNEstep = nNEi / fcPerSide(2).toFloat
...
//      }
    ...
    for(i <- 0 until nSWo){
      for (j <- 0 until fcPerSide(0)) {
        if (nNWi > 0) {
          val fcNWstep = nNWi / fcPerSide(0).toFloat
      ...
      for (j <- 0 until fcPerSide(1)) {
        if (nNEi > 0) {
          val fcNEstep = nNEi / fcPerSide(1).toFloat
      ...
      for (j <- 0 until fcPerSide(2)) {
        if (nSEi > 0) {
          val fcSEstep = nSEi / fcPerSide(2).toFloat
...
//      }
   ...
   * @param track2pin     track-to-pin connection flexibility
   */
  def getGIBConnect(numTrack: Int, numIOPinList: List[Int], pin2pin: List[Int], pin2track:List[Int], track2pin:List[Int], trackDirections : ListBuffer[Int]): ListBuffer[(Seq[Int], ListBuffer[Seq[Int]])] = {//@HETA:  ListBuffer[(Seq[Int], ListBuffer[Seq[Int]])] is return value
    var dstList = ListBuffer[Seq[Int]]()
    var srcsList = ListBuffer[ListBuffer[Seq[Int]]]() // srcs connected to each dst
    val oPin2IPinConnect = getOPin2IPinConnect(numIOPinList, pin2pin)
    ...
    if(numTrack > 0) {
      val track2IPinConnect = getTrack2IPinConnect(numTrack, List(numIOPinList(0), numIOPinList(2), numIOPinList(4), numIOPinList(6)), track2pin, trackDirections)//0,2,4,6, the input pins of PE/LSU
      ...
      val oPin2TrackConnect = getOPin2TrackConnect(numTrack, List(numIOPinList(1), numIOPinList(3), numIOPinList(5), numIOPinList(7)), pin2track, trackDirections)
...
class GIB(attrs: mutable.Map[String, Any]) extends Module with IR {
  ...
  val pin2pin = getAttrValue("opin_2_ipin_connect_flexibility").asInstanceOf[Map[String, Int]]
  val num_ipin_side1 = pin2pin("num_ipin_side1")
  val num_ipin_side2 = pin2pin("num_ipin_side2")
  val num_ipin_side3 = pin2pin("num_ipin_side3")
  val pin2pinList = List(num_ipin_side1, num_ipin_side2, num_ipin_side3)
  val pin2track = getAttrValue("opin_2_otrack_connect_flexibility").asInstanceOf[Map[String, Int]]
  val num_otrack_side1 = pin2track("num_otrack_side1")
  val num_otrack_side2 = pin2track("num_otrack_side2")
  val num_otrack_side3 = pin2track("num_otrack_side3")
  val num_otrack_side4 = pin2track("num_otrack_side4")
  val pin2trackList = List(num_otrack_side1, num_otrack_side2, num_otrack_side3, num_otrack_side4)
  val track2pin = getAttrValue("itrack_2_ipin_connect_flexibility").asInstanceOf[Map[String, Int]]
  val num_itrack_side1 = track2pin("num_itrack_side1")
  val num_itrack_side2 = track2pin("num_itrack_side2")
  val num_itrack_side3 = track2pin("num_itrack_side3")
  val num_itrack_side4 = track2pin("num_itrack_side4")
  val track2pinList = List(num_itrack_side1, num_itrack_side2, num_itrack_side3, num_itrack_side4)
  // numIOPinList
  ...
  // fcList
//  val fcMap = getAttrValue("connect_flexibility").asInstanceOf[Map[String, Int]]
...
//@HETA: initial value of outports(0)
  io.ipinNW.map{ _ := 0.U }
  io.ipinNE.map{ _ := 0.U }
  io.ipinSW.map{ _ := 0.U }
  io.ipinSE.map{ _ := 0.U }
  ...
  // wire connections
  val connect = GIB.getGIBConnect(numTrack, numIOPinList, pin2pinList, pin2trackList, track2pinList, trackDirections)
