...
//@Yuan: after changing the config delay, the cnt will be reset
//so the DelayPipe can be reused
class DelayPipe(width: Int, maxDelay: Int) extends Module {
  val cfgWidth = log2Ceil(maxDelay+1)
  val io = IO(new Bundle {
    val en = Input(Bool())
  ...
  val wptr = RegInit(0.U(cfgWidth.W))   // write pointer
  val rptr = RegInit(0.U(cfgWidth.W))   // read pointer
  //@Yuan: if we change the config delay, it needs to reset the cnt
  val config_temp = RegInit(0.U(cfgWidth.W))   // read pointer
  ...
  when(!io.en){
    cnt := 0.U
  }.elsewhen(config_temp =/= io.config){
    cnt := 1.U
  }.elsewhen(cnt < io.config){
    cnt := cnt + 1.U
  }
